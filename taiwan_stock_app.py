...
if 'strong_stocks' in st.session_state:
    ...
    if target_stock:
        ...
        if not df_stock.empty:
            ...
            # 儀表板
            cur_p, b20 = float(df_stock['Close'].iloc[-1]), float(df_stock['BIAS_20'].iloc[-1])
            m1, m2 = st.columns(2)
            m1.metric("當前股價", f"{cur_p:.2f}")
            m2.metric("20日乖離率", f"{b20:.2f}%", delta="過熱" if b20 > 10 else "正常", delta_color="inverse" if b20 > 10 else "normal")
            ...
            fig.add_trace(go.Candlestick(
                ...
                increasing_line_color='#FF0000', decreasing_line_color='#008000', # 紅漲綠跌
                increasing_fillcolor='#FF0000', decreasing_fillcolor='#008000'
            ), row=1, col=1)
            
            ma_cfg = [('MA5','blue','5MA'),('MA20','orange','20MA'),('MA60','green','60MA'),('MA120','purple','120MA'),('MA240','black','240MA')]
            ...
            # (B) 成交量 - 紅漲綠跌
            v_colors = ['#FF0000' if c >= o else '#008000' for c, o in zip(plot_df['Close'], plot_df['Open'])]
            fig.add_trace(go.Bar(x=plot_df.index, y=plot_df['Volume'], name='成交量', marker_color=v_colors), row=2, col=1)

            # (C) MACD - 紅漲綠跌
            fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['DIF'], name='DIF', line=dict(color='gray')), row=3, col=1)
            fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['MACD_L'], name='MACD', line=dict(color='orange')), row=3, col=1)
            o_colors = ['#FF0000' if x >= 0 else '#008000' for x in plot_df['OSC']]
